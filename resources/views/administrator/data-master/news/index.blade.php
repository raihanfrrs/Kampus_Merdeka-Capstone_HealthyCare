...
<div class="card mb-4">
    <div class="card-header">
        ...
        Master News
        <a href="/news/create" class="btn btn-secondary btn-sm float-end">
            <svg style="width:24px;height:24px" viewBox="0 0 24 24" class="me-1">
    ...
    <div class="card-body">
        <table id="datatablesSimple">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $item->title }}</td>
                    <td>{{ $item->categorynews->category }}</td>
                    <td>{{ $item->author }}</td>
                    <td>
                        <form action="/news/{{ $item->id }}" method="post">
                            @csrf
                            @method('delete')
                            <a href="/news/{{ $item->id }}/edit" class="btn btn-warning btn-sm text-white">
