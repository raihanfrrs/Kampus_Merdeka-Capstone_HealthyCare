<div class="p-5 rounded-lg jumbotron">
    <div class="row g-0">
        <div class="col-md-12 col-lg-12 col-xl-8">
            <h4><p class="fw-semibold text-info">EASY TO HEALTHY, EASY KNOW U DISEASE</p></h4>
            <h1 class="display-4">We Follow a Holistic Approach to HealthyCare.</h1>
            <p class="lead">This is a simple hero unit, a simple jumbotron-style component for calling extra attention to featured content or information.</p>
            <p>It uses utility classes for typography and spacing to space content out within the larger container.</p>
            <a class="btn btn-primary btn-lg" href="konsultasi" role="button">Get Started!</a>
        </div>
        <div class="col-xl-4">
            <img src="{{ asset('/') }}assets/img/background-image-3.jpg" alt="background-3" class="img-fluid w-75">
        </div>
    </div>
</div>

<div class="p-5 rounded-lg bg-light">
    <div class="row g-0 d-flex justify-content-center">
        <p class="text-center fs-2 fw-semibold text-info">Why HealthyCare?</p>
        <div class="card mx-5 my-2 border-0 bg-transparent" style="width: 18rem;">
            <div class="card-header border-0 fs-1 bg-transparent">
                <svg style="width:48px;height:48px" viewBox="0 0 24 24">
                    <path fill="currentColor" d="M21.71 20.29L20.29 21.71A1 1 0 0 1 18.88 21.71L7 9.85A3.81 3.81 0 0 1 6 10A4 4 0 0 1 2.22 4.7L4.76 7.24L5.29 6.71L6.71 5.29L7.24 4.76L4.7 2.22A4 4 0 0 1 10 6A3.81 3.81 0 0 1 9.85 7L21.71 18.88A1 1 0 0 1 21.71 20.29M2.29 18.88A1 1 0 0 0 2.29 20.29L3.71 21.71A1 1 0 0 0 5.12 21.71L10.59 16.25L7.76 13.42M20 2L16 4V6L13.83 8.17L15.83 10.17L18 8H20L22 4Z" />
                </svg>
            </div>
            <div class="card-body">
                <h6 class="card-title fw-semibold">Easy to Use</h6>
                <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
            </div>
        </div>
        <div class="card mx-5 my-2 border-0 bg-transparent" style="width: 18rem;">
            <div class="card-header border-0 fs-1 bg-transparent">
                <svg style="width:48px;height:48px" viewBox="0 0 24 24">
                    <path fill="currentColor" d="M12,2A10,10 0 0,0 2,12A10,10 0 0,0 12,22A10,10 0 0,0 22,12C22,10.84 21.79,9.69 21.39,8.61L19.79,10.21C19.93,10.8 20,11.4 20,12A8,8 0 0,1 12,20A8,8 0 0,1 4,12A8,8 0 0,1 12,4C12.6,4 13.2,4.07 13.79,4.21L15.4,2.6C14.31,2.21 13.16,2 12,2M19,2L15,6V7.5L12.45,10.05C12.3,10 12.15,10 12,10A2,2 0 0,0 10,12A2,2 0 0,0 12,14A2,2 0 0,0 14,12C14,11.85 14,11.7 13.95,11.55L16.5,9H18L22,5H19V2M12,6A6,6 0 0,0 6,12A6,6 0 0,0 12,18A6,6 0 0,0 18,12H16A4,4 0 0,1 12,16A4,4 0 0,1 8,12A4,4 0 0,1 12,8V6Z" />
                </svg>
            </div>
            <div class="card-body">
                <h6 class="card-title fw-semibold">Accurate</h6>
                <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
            </div>
        </div>
        <div class="card mx-5 my-2 border-0 bg-transparent" style="width: 18rem;">
            <div class="card-header border-0 fs-1 bg-transparent">
                <svg style="width:48px;height:48px" viewBox="0 0 24 24">
                    <path fill="currentColor" d="M3,4.27L4.28,3L21,19.72L19.73,21L16.06,17.33C15.44,18 14.54,18.55 13.5,18.82V21H10.5V18.82C8.47,18.31 7,16.79 7,15H9C9,16.08 10.37,17 12,17C13.13,17 14.14,16.56 14.65,15.92L11.68,12.95C9.58,12.42 7,11.75 7,9C7,8.77 7,8.55 7.07,8.34L3,4.27M10.5,5.18V3H13.5V5.18C15.53,5.69 17,7.21 17,9H15C15,7.92 13.63,7 12,7C11.63,7 11.28,7.05 10.95,7.13L9.4,5.58L10.5,5.18Z" />
                </svg>
            </div>
            <div class="card-body">
                <h6 class="card-title fw-semibold">Free</h6>
                <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
            </div>
        </div>
    </div>
</div>

<div class="p-5 rounded-lg">
    <div class="row g-0">
        <div class="col-lg-5">
            <img src="{{ asset('/') }}assets/img/background-image-4.png" alt="" class="img-fluid w-100">
        </div>
        <div class="col-lg-7 bg-info p-5">
            <p>
                Lorem ipsum dolor sit amet, consectetur adipiscing elit. Duis dignissim sodales vulputate. Donec elementum erat a ex fringilla, tempus viverra sapien condimentum. Vestibulum nec diam luctus, scelerisque nisl at, suscipit purus. Quisque sit amet arcu sit amet neque tincidunt vehicula ac eu sem. Sed id molestie arcu. Aenean porta faucibus odio sit amet porta. Maecenas quis finibus dui, at scelerisque leo. Maecenas et commodo nibh. In hac habitasse platea dictumst. Aliquam eget viverra dui.
            </p>

            <p>
                Integer aliquam eget diam eu fermentum. Nulla et enim ut arcu venenatis pellentesque a a sapien. Duis vehicula, risus nec imperdiet congue, nulla mauris pellentesque quam, sed faucibus quam quam a est. Morbi aliquam aliquet vestibulum. Nunc eu arcu non nulla molestie viverra vel non dui. Phasellus id sem at justo euismod fermentum. Nullam consectetur quam nisi, sit amet laoreet eros congue sed.
            </p>

            <p>
                In euismod eget massa ut sodales. Donec ut ipsum magna. Etiam sollicitudin elit non orci imperdiet lobortis. Nam sagittis eu erat a finibus. Praesent sed malesuada velit. Etiam quis diam id nibh consequat accumsan. Morbi ut scelerisque odio, nec vulputate nisi. Sed vitae sagittis arcu.
            </p>
        </div>
    </div>
</div>

<div class="p-5 rounded-lg bg-light">
    <div class="row g-0 d-flex justify-content-center">
        <p class="text-center fs-2 fw-semibold text-info">Our Service</p>
        <div class="card mx-5 my-2 border-0 bg-transparent" style="width: 28rem;">
            <div class="card-header border-0 fs-1 bg-transparent">
                <svg style="width:48px;height:48px" viewBox="0 0 24 24">
                    <path fill="currentColor" d="M21.71 20.29L20.29 21.71A1 1 0 0 1 18.88 21.71L7 9.85A3.81 3.81 0 0 1 6 10A4 4 0 0 1 2.22 4.7L4.76 7.24L5.29 6.71L6.71 5.29L7.24 4.76L4.7 2.22A4 4 0 0 1 10 6A3.81 3.81 0 0 1 9.85 7L21.71 18.88A1 1 0 0 1 21.71 20.29M2.29 18.88A1 1 0 0 0 2.29 20.29L3.71 21.71A1 1 0 0 0 5.12 21.71L10.59 16.25L7.76 13.42M20 2L16 4V6L13.83 8.17L15.83 10.17L18 8H20L22 4Z" />
                </svg>
            </div>
            <div class="card-body">
                <h6 class="card-title fw-semibold">Easy to Use</h6>
                <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
            </div>
        </div>
        <div class="card mx-5 my-2 border-0 bg-transparent" style="width: 28rem;">
            <div class="card-header border-0 fs-1 bg-transparent">
                <svg style="width:48px;height:48px" viewBox="0 0 24 24">
                    <path fill="currentColor" d="M12,2A10,10 0 0,0 2,12A10,10 0 0,0 12,22A10,10 0 0,0 22,12C22,10.84 21.79,9.69 21.39,8.61L19.79,10.21C19.93,10.8 20,11.4 20,12A8,8 0 0,1 12,20A8,8 0 0,1 4,12A8,8 0 0,1 12,4C12.6,4 13.2,4.07 13.79,4.21L15.4,2.6C14.31,2.21 13.16,2 12,2M19,2L15,6V7.5L12.45,10.05C12.3,10 12.15,10 12,10A2,2 0 0,0 10,12A2,2 0 0,0 12,14A2,2 0 0,0 14,12C14,11.85 14,11.7 13.95,11.55L16.5,9H18L22,5H19V2M12,6A6,6 0 0,0 6,12A6,6 0 0,0 12,18A6,6 0 0,0 18,12H16A4,4 0 0,1 12,16A4,4 0 0,1 8,12A4,4 0 0,1 12,8V6Z" />
                </svg>
            </div>
            <div class="card-body">
                <h6 class="card-title fw-semibold">Accurate</h6>
                <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
            </div>
        </div>
        <div class="card mx-5 my-2 border-0 bg-transparent" style="width: 28rem;">
            <div class="card-header border-0 fs-1 bg-transparent">
                <svg style="width:48px;height:48px" viewBox="0 0 24 24">
                    <path fill="currentColor" d="M3,4.27L4.28,3L21,19.72L19.73,21L16.06,17.33C15.44,18 14.54,18.55 13.5,18.82V21H10.5V18.82C8.47,18.31 7,16.79 7,15H9C9,16.08 10.37,17 12,17C13.13,17 14.14,16.56 14.65,15.92L11.68,12.95C9.58,12.42 7,11.75 7,9C7,8.77 7,8.55 7.07,8.34L3,4.27M10.5,5.18V3H13.5V5.18C15.53,5.69 17,7.21 17,9H15C15,7.92 13.63,7 12,7C11.63,7 11.28,7.05 10.95,7.13L9.4,5.58L10.5,5.18Z" />
                </svg>
            </div>
            <div class="card-body">
                <h6 class="card-title fw-semibold">Free</h6>
                <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
            </div>
        </div>
        <div class="card mx-5 my-2 border-0 bg-transparent" style="width: 28rem;">
            <div class="card-header border-0 fs-1 bg-transparent">
                <svg style="width:48px;height:48px" viewBox="0 0 24 24">
                    <path fill="currentColor" d="M3,4.27L4.28,3L21,19.72L19.73,21L16.06,17.33C15.44,18 14.54,18.55 13.5,18.82V21H10.5V18.82C8.47,18.31 7,16.79 7,15H9C9,16.08 10.37,17 12,17C13.13,17 14.14,16.56 14.65,15.92L11.68,12.95C9.58,12.42 7,11.75 7,9C7,8.77 7,8.55 7.07,8.34L3,4.27M10.5,5.18V3H13.5V5.18C15.53,5.69 17,7.21 17,9H15C15,7.92 13.63,7 12,7C11.63,7 11.28,7.05 10.95,7.13L9.4,5.58L10.5,5.18Z" />
                </svg>
            </div>
            <div class="card-body">
                <h6 class="card-title fw-semibold">Free</h6>
                <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
            </div>
        </div>
        <div class="card mx-5 my-2 border-0 bg-transparent" style="width: 28rem;">
            <div class="card-header border-0 fs-1 bg-transparent">
                <svg style="width:48px;height:48px" viewBox="0 0 24 24">
                    <path fill="currentColor" d="M3,4.27L4.28,3L21,19.72L19.73,21L16.06,17.33C15.44,18 14.54,18.55 13.5,18.82V21H10.5V18.82C8.47,18.31 7,16.79 7,15H9C9,16.08 10.37,17 12,17C13.13,17 14.14,16.56 14.65,15.92L11.68,12.95C9.58,12.42 7,11.75 7,9C7,8.77 7,8.55 7.07,8.34L3,4.27M10.5,5.18V3H13.5V5.18C15.53,5.69 17,7.21 17,9H15C15,7.92 13.63,7 12,7C11.63,7 11.28,7.05 10.95,7.13L9.4,5.58L10.5,5.18Z" />
                </svg>
            </div>
            <div class="card-body">
                <h6 class="card-title fw-semibold">Free</h6>
                <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
            </div>
        </div>
        <div class="card mx-5 my-2 border-0 bg-transparent" style="width: 28rem;">
            <div class="card-header border-0 fs-1 bg-transparent">
                <svg style="width:48px;height:48px" viewBox="0 0 24 24">
                    <path fill="currentColor" d="M3,4.27L4.28,3L21,19.72L19.73,21L16.06,17.33C15.44,18 14.54,18.55 13.5,18.82V21H10.5V18.82C8.47,18.31 7,16.79 7,15H9C9,16.08 10.37,17 12,17C13.13,17 14.14,16.56 14.65,15.92L11.68,12.95C9.58,12.42 7,11.75 7,9C7,8.77 7,8.55 7.07,8.34L3,4.27M10.5,5.18V3H13.5V5.18C15.53,5.69 17,7.21 17,9H15C15,7.92 13.63,7 12,7C11.63,7 11.28,7.05 10.95,7.13L9.4,5.58L10.5,5.18Z" />
                </svg>
            </div>
            <div class="card-body">
                <h6 class="card-title fw-semibold">Free</h6>
                <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
            </div>
        </div>
    </div>
</div>

<div class="p-5 rounded-lg">
    <div class="row g-0 d-flex justify-content-center">
        <p class="text-center fs-2 fw-semibold text-info">Berita Seputar Kesehatan</p>
        <div class="card shadow border-0 mx-5 my-4" style="width: 18rem;">
            <img src="{{ asset('/') }}assets/img/background-image-5.png" class="card-img-top" alt="...">
            <div class="card-body">
              <span class="fw-light fs-6 font-monospace text-uppercase" style="letter-spacing: .1rem;">Sistem Pernafasan</span>
              <h5 class="card-title fw-semibold">Card title</h5>
              <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
              <p class="card-text font-monospace fw-bolder text-primary"><a href="berita/slug" class="text-decoration-none">Read Here <i class="bi bi-arrow-right" style="vertical-align: -.150rem"></i></a></p>
            </div>
        </div>
        <div class="card shadow border-0 mx-5 my-4" style="width: 18rem;">
            <img src="{{ asset('/') }}assets/img/background-image-5.png" class="card-img-top" alt="...">
            <div class="card-body">
              <span class="fw-light fs-6 font-monospace text-uppercase" style="letter-spacing: .1rem;">Sistem Pernafasan</span>
              <h5 class="card-title fw-semibold">Card title</h5>
              <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
              <p class="card-text font-monospace fw-bolder text-primary"><a href="berita/slug" class="text-decoration-none">Read Here <i class="bi bi-arrow-right" style="vertical-align: -.150rem"></i></a></p>
            </div>
        </div>
        <div class="card shadow border-0 mx-5 my-4" style="width: 18rem;">
            <img src="{{ asset('/') }}assets/img/background-image-5.png" class="card-img-top" alt="...">
            <div class="card-body">
              <span class="fw-light fs-6 font-monospace text-uppercase" style="letter-spacing: .1rem;">Sistem Pernafasan</span>
              <h5 class="card-title fw-semibold">Card title</h5>
              <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
              <p class="card-text font-monospace fw-bolder text-primary"><a href="berita/slug" class="text-decoration-none">Read Here <i class="bi bi-arrow-right" style="vertical-align: -.150rem"></i></a></p>
            </div>
        </div>
        <div class="card shadow border-0 mx-5 my-4" style="width: 18rem;">
            <img src="{{ asset('/') }}assets/img/background-image-5.png" class="card-img-top" alt="...">
            <div class="card-body">
              <span class="fw-light fs-6 font-monospace text-uppercase" style="letter-spacing: .1rem;">Sistem Pernafasan</span>
              <h5 class="card-title fw-semibold">Card title</h5>
              <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
              <p class="card-text font-monospace fw-bolder text-primary"><a href="berita/slug" class="text-decoration-none">Read Here <i class="bi bi-arrow-right" style="vertical-align: -.150rem"></i></a></p>
            </div>
        </div>
        <div class="card shadow border-0 mx-5 my-4" style="width: 18rem;">
            <img src="{{ asset('/') }}assets/img/background-image-5.png" class="card-img-top" alt="...">
            <div class="card-body">
              <span class="fw-light fs-6 font-monospace text-uppercase" style="letter-spacing: .1rem;">Sistem Pernafasan</span>
              <h5 class="card-title fw-semibold">Card title</h5>
              <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
              <p class="card-text font-monospace fw-bolder text-primary"><a href="berita/slug" class="text-decoration-none">Read Here <i class="bi bi-arrow-right" style="vertical-align: -.150rem"></i></a></p>
            </div>
        </div>
        <a href="berita" class="text-center">Read All News.</a>
    </div>
</div>